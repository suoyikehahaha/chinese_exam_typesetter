"""Production Windows launcher for v0.4.3."""
...
def _load_application() -> object:
    ...
    return importlib.import_module("desktop_app_v043")
...
def main() -> int:
    ...
    if "--font-test-output" in sys.argv:
        output = Path(sys.argv[sys.argv.index("--font-test-output") + 1])
        return _run_font_test(output)
