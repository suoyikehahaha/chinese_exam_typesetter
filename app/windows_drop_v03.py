    ...
    try:
        ...
        def poll_pending() -> None:
            ...
            for paths in batches:
                callback(paths)
            if getattr(widget, "_exam_drop_state_v03", None):
                poll_token = widget.after(80, poll_pending)
                widget._exam_drop_state_v03["poll_token"] = poll_token
        ...
        if not registered:
            return False
        setattr(widget, "_exam_drop_state_v03", {
...
def uninstall_file_drop(widget: object) -> None:
    state = getattr(widget, "_exam_drop_state_v03", None)
    if not state:
        return
    ...
    for hwnd in handles:
        try:
            ...
            pass
    try:
        delattr(widget, "_exam_drop_state_v03")
