param(
    [string]$PythonPath = "",
    [string]$DistPath = "release-v045-final"
)
...
$pyInstallerCache = Join-Path $root "work\pyinstaller-v04"
...
try {
    & $PythonPath -m PyInstaller --noconfirm --clean `
        ...
        --workpath (Join-Path $root "work\build-v045-final") `
        (Join-Path $root "ChineseExamTypesetter_v045_final.spec")
