"""Production Windows launcher for the v0.4 release."""
...
def _load_application() -> object:
    ...
    return importlib.import_module("desktop_app_v04_final")


def _run_import_test(module: object, source: Path, output: Path) -> int:
    data = module.stable.release.current_v02.import_exam(source)
    result = {
        "source": source.name,
...
def _run_startup_test(module: object, output: Path) -> int:
    ...
    def finish() -> None:
        ...
        first = next(
            (iid for iid in app.tree.get_children(roots[0]) if str(iid).startswith("block-")),
            None,
        ) if roots else None
        if first:
        ...
        payload = {
            ...
            "tree": bool(roots),
            "objects": len(app._content_objects),
